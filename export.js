...
function getSelectedFields() {
    const fieldMap = {
        ...
        'export-field-critico': { key: 'passoCritico', label: 'Passo Crítico', width: 45 },
        'export-field-proximos': { key: 'proximos_passos', label: 'Próximos Passos', width: 50 },
        'export-field-aguardando': { key: 'aguardando_resposta', label: 'Aguardando Resposta', width: 45 },
...
function getFieldValue(project, key) {
    // Campo especial: aguardando_resposta (extraído de proximos_passos)
    if (key === 'aguardando_resposta') {
        return formatAguardandoResposta(project.proximos_passos);
    }
...
// Formatar passos que estão aguardando resposta
function formatAguardandoResposta(steps) {
    ...
    if (waitingSteps.length === 0) return '';
    
    return waitingSteps.map((step, index) => {
        const texto = step.texto || '';
        const aguardando = step.aguardando || 'Não especificado';
        const aguardandoDesde = step.aguardandoDesde;
        
        let diasEsperando = '';
        if (aguardandoDesde) {
            const dias = Math.floor((new Date() - new Date(aguardandoDesde)) / (1000 * 60 * 60 * 24));
            diasEsperando = ` (${dias} dias)`;
        }
        
        return `${index + 1}. ${texto} → Aguardando: ${aguardando}${diasEsperando}`;
    }).join('\n');
}
...
function formatProximosPassos(steps, format = 'text') {
    ...
    return steps.map((step, index) => {
        ...
        const aguardando = step.aguardando || '';
...
function exportProjectActionsXLSX(project, steps, fileName) {
    ...
    const headers = ['Nº', 'Ação', 'Status', 'Aguardando', 'Prazo', 'Responsável'];
    const data = [headers];
    ...
    steps.forEach((step, index) => {
        ...
        data.push([index + 1, texto, statusLabel, aguardando, prazo, responsavel]);
    });
...
function exportProjectActionsPDF(project, steps, fileName) {
    ...
    // Passo Crítico (se existir)
    if (project.passoCritico) {
        doc.setFontSize(10);
        doc.setTextColor(244, 33, 46);
        doc.setFont(undefined, 'bold');
        doc.text('🎯 Passo Crítico:', margin, 36);
        doc.setTextColor(50, 50, 50);
        doc.setFont(undefined, 'normal');
        const criticalLines = doc.splitTextToSize(project.passoCritico, pageWidth - 2 * margin);
        doc.text(criticalLines, margin, 42);
    }

    // Preparar dados
    const startY = project.passoCritico ? 52 : 35;
    const headers = ['Nº', 'Ação', 'Status', 'Aguardando', 'Prazo', 'Resp.'];
    const tableData = steps.map((step, index) => {
        ...
        return [String(index + 1), texto, statusLabel, aguardando, prazo, responsavel];
    });
...
function exportProjectActionsCSV(project, steps, fileName) {
    ...
    const rows = steps.map((step, index) => {
        ...
        return [index + 1, `"${texto.replace(/"/g, '""')}"`, statusLabel, `"${aguardando.replace(/"/g, '""')}"`, prazo, responsavel].join(';');
    });
